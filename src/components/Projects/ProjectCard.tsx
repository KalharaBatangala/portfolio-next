...
const ProjectCard: React.FC<{ project: Project }> = ({ project }) => {
  return (
    <motion.div
      ...
      style={{ backgroundImage: `url('/assets/titanium.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center' }}
    ...
    >
      ...
      <div className="flex space-x-4">
        {project.github.map((link, index) => (
          <Link key={index} href={link} target="_blank" className="text-orange-500 hover:text-orange-400">
            GitHub
          </Link>
        ))}
      </div>
      <motion.div
        ...
        initial={{ background: 'linear-gradient(to right, rgba(255, 255, 255, 0) 0%, rgba(255, 255, 255, 0) 100%)' }}
        whileHover={{
          background: 'linear-gradient(to right, rgba(255, 255, 255, 0.95) 0%, rgba(255, 255, 255, 0) 70%)',
        }}
